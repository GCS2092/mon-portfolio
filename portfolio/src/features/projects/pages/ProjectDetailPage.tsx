...
function StackCard({ tech, index }: { tech: ProjectTech; index: number }) {
      ...
      <div className="group p-6 rounded-2xl border border-[#E8E6E1] bg-white hover:shadow-lg hover:border-[#2D5BE3]/30 transition-all duration-300">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="font-mono text-xs px-2.5 py-1 rounded-full border border-[#E8E6E1] text-[#6B6B6B] bg-[#FAF9F7]">
            {getTechRole(tech, lang)}
          </span>
        </div>
        <p className="text-sm text-[#6B6B6B] leading-relaxed mb-4">
          {getTechDescription(tech, lang)}
        </p>
        <div className="pt-4 border-t border-[#E8E6E1]">
          <p className="font-mono text-xs text-[#2D5BE3] mb-1.5">
            {t('project.tech.howItWorks')}
          </p>
          <p className="text-sm text-[#1A1A1A] leading-relaxed">{getTechHow(tech, lang)}</p>
        </div>
      </div>
    </div>
  )
...
export default function ProjectDetailPage() {
  ...
  if (!project) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center">
        <p className="font-mono text-sm text-[#6B6B6B] mb-4">{t('project.notFound')}</p>
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F7]">
      <section
        ref={heroRef}
        className={`pt-32 pb-16 max-w-5xl mx-auto px-6 transition-all duration-700 ${
      ...
      >
        ...
        <p className="text-base md:text-lg text-[#6B6B6B] max-w-2xl leading-relaxed mb-8">
          {getProjectFullDescription(project, lang)}
        </p>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {project.tags.map((tag) => (
            <span
              key={tag}
              className="font-mono text-xs px-3 py-1.5 rounded-full border border-[#E8E6E1] bg-white text-[#6B6B6B]"
            >
              {tag}
            </span>
          ))}
